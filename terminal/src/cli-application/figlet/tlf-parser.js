/**
 * TlfParser - A class for parsing tlf files
 */
export class TlfParser {
    /**
     * Parses the TLF font file
    ...
    parse(fontFile) {
        ...
        // Parse header line: tlf2a$ height baseline maxLength oldLayout commentLines printDirection fullLayout codeTagCount
        const headerLine = lines[0];
        if (!headerLine.startsWith('tlf2a')) {
            throw new Error('Invalid TLF file format');
        }
        ...
        while (lineIndex < lines.length) {
            ...
            for (let i = 0; i < height && lineIndex < lines.length; i++) {
                ...
                // TLF format: each line ends with @/#.., last line of character ends with 2 of specified characters
